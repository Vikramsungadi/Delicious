	...
	return (
		<nav className="flex w-full items-center justify-between border-b  border-gray  py-4 max-md:py-0 max-sm:px-2 max-sm:py-0 ">
			<div className="image my-4 h-[6rem] shrink-0 translate-y-4 max-md:h-[5.5rem]">
				<NavLink to={"/"}>
					<img src={logo} className="h-full  max-md:h-[70%]" alt="Delicious Restaurant Logo" />
				</NavLink>
			</div>
			...
			<ul
				css={StyledUl}
				className={`gap-24 font-poppins text-lg font-[400] max-lg:gap-16 max-lg:text-md max-lg:font-medium max-md:z-10 max-sm:translate-y-[0.75rem] ${
					openMenu ? "" : "max-md:!hidden"
				} `}>
				<li className={`fixed right-0 top-0 self-end p-12 ${!openMenu ? "md:hidden" : ""}`}>
					<HiX className=" text-xl md:hidden" onClick={toggleMenu} />
				</li>
